...
const actions = {
	 ...
	 * @param args
	 */
	async get(context: ActionContext<any, any>, args: paramsType) {
		const { api, params, headers } = args;
		const res = await request.get(api, { params, headers });
		return res;
	},
	/**
	 * post 方法
	...
	async delete(context: ActionContext<any, any>, args: dataType) {
		const { api, data, type } = args;
		const res = await request.delete(api, { data, type } as MyAxiosRequestConfig);
		return res;
	},
	/**
	 * put 方法
